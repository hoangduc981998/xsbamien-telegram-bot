...
class TestGetLatestResult:
    ...
    async def test_get_latest_result_success(
        ...
        # Mock API client
        lottery_service.api_client.fetch_results = AsyncMock(return_value=sample_api_response)

        # Mock transformer
        lottery_service.transformer.transform_results = Mock(return_value=[sample_transformed_result])
    ...
    @pytest.mark.asyncio
    async def test_get_latest_result_api_failure_returns_mock(self, lottery_service, mocker):
    ...
    @pytest.mark.asyncio
    async def test_get_latest_result_empty_results_returns_mock(self, lottery_service, sample_api_response, mocker):
    ...
    @pytest.mark.asyncio
    async def test_get_latest_result_exception_returns_mock(self, lottery_service, mocker):
        """Test that mock data is returned when exception occurs"""
        # Mock API client to raise exception
        lottery_service.api_client.fetch_results = AsyncMock(side_effect=Exception("API Error"))
    ...
    @pytest.mark.asyncio
    async def test_get_latest_result_multiple_results_returns_first(self, lottery_service, sample_api_response, mocker):
...
class TestGetHistory:
    ...
    @pytest.mark.asyncio
    async def test_get_history_success(self, lottery_service, sample_api_response, mocker):
    ...
    async def test_get_history_default_limit(self, lottery_service, mocker):
        ...
        lottery_service.api_client.fetch_results = AsyncMock(return_value={"success": True, "t": {"issueList": []}})
        lottery_service.transformer.transform_results = Mock(return_value=[])
    ...
    @pytest.mark.asyncio
    async def test_get_history_custom_limit(self, lottery_service, sample_api_response, mocker):
    ...
    @pytest.mark.asyncio
    async def test_get_history_returns_all_results(self, lottery_service, sample_api_response, mocker):
        """Test that all transformed results are returned"""
        results = [{"date": f"2025-10-{14-i:02d}", "province": "Test"} for i in range(10)]
    ...
    @pytest.mark.asyncio
    async def test_get_history_different_provinces(self, lottery_service, sample_api_response, mocker):
...
class TestServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_service_transformer_called_correctly(self, lottery_service, sample_api_response, mocker):
        """Test that transformer is called with API response"""
        lottery_service.api_client.fetch_results = AsyncMock(return_value=sample_api_response)
        lottery_service.transformer.transform_results = Mock(return_value=[{"date": "2025-10-14"}])

        await lottery_service.get_latest_result("MB")

        # Verify transformer was called with API response
        lottery_service.transformer.transform_results.assert_called_once_with(sample_api_response)

    @pytest.mark.asyncio
    async def test_service_reusable(self, lottery_service, sample_api_response, sample_transformed_result, mocker):
